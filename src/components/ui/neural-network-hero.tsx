...
function ShaderBackground() {
  ...
  return (
    <div ref={canvasRef} className="absolute inset-0 w-full h-full">
      <Canvas camera={camera} style={{ width: '100%', height: '100%' }} gl={{ antialias: false, alpha: false }}>
        <ShaderPlane />
      </Canvas>
    </div>
  );
...
export default function Hero({
  ...
  microDetails = ["150 Problems", "18 Categories", "C++ Solutions"]
}: HeroProps) {
  ...
  const microItem1Ref = useRef<HTMLSpanElement>(null);
  const microItem2Ref = useRef<HTMLSpanElement>(null);
  const microItem3Ref = useRef<HTMLSpanElement>(null);
  ...
  return (
    <section ref={sectionRef} className="relative w-full min-h-[90vh] flex items-center justify-center overflow-hidden">
      <ShaderBackground />

      <div className="relative z-10 flex flex-col items-center text-center px-4 max-w-3xl mx-auto">
        <div
          ref={badgeRef}
          className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-primary/30 bg-primary/10 backdrop-blur-sm mb-8"
        >
          <span className="text-xs font-semibold text-primary uppercase tracking-wider">{badgeLabel}</span>
          <span className="w-px h-3 bg-primary/30" />
          <span className="text-xs text-foreground/80">{badgeText}</span>
        </div>
        ...
        <p
          ref={paraRef}
          className="text-lg md:text-xl text-foreground/70 max-w-xl leading-relaxed mb-10"
        >
          {description}
        </p>
        ...
        <div ref={ctaRef} className="flex flex-col sm:flex-row gap-3 mb-10">
          {ctaButtons.map((button, index) => (
            <a
              ...
              className={
                button.primary
                  ? 'inline-flex items-center justify-center px-6 py-3 rounded-md text-sm font-medium bg-primary text-primary-foreground hover:bg-primary/90 transition-colors'
                  : 'inline-flex items-center justify-center px-6 py-3 rounded-md text-sm font-medium border border-border bg-secondary/50 text-secondary-foreground hover:bg-secondary transition-colors backdrop-blur-sm'
              }
            >
              {button.text}
            </a>
        ...
        <div ref={microRef} className="flex items-center gap-4">
